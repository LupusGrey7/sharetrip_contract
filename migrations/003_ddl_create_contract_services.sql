...
CREATE TABLE IF NOT EXISTS contract_management.services (
    service_code VARCHAR(32) NOT NULL DEFAULT 'trip_creation', -- 'trip_creation', 'trip_participants', 'notifications', 'premium_support'
    description TEXT NOT NULL DEFAULT 'создание поездок', -- 'создание поездок', 'добавление участников поездки', 'отправка уведомлений', 'extended support'
...
CREATE TABLE IF NOT EXISTS contract_management.contract_services (
    ...
    PRIMARY KEY (id),
    CONSTRAINT fk_contract_services_service_code FOREIGN KEY (service_code) REFERENCES contract_management.services(service_code),
    CONSTRAINT fk_contract_services_contract_id FOREIGN KEY (contract_id) REFERENCES contract_management.contracts(id)
);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_contract_services_unique ON contract_management.contract_services (contract_id, service_code);
